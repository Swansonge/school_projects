...
double PolyCircumference(Polygon &thePoly) {
	...
	for (i = 0;i < thePoly.numSides; i++) {

		//starting and ending points of the polygon side
		Point pt1 = thePoly.v[i];
		Point pt2 = thePoly.v[(i + 1) % thePoly.numSides];

		//add the next side length
		cirCount += pt1.Length(pt2);
	}
...
void PolyArea(Polygon &thePoly, double *area) {
	...
	area[0] = 0;
	area[1] = 0;
	Point pt1 = thePoly.v[0];
	Point pt2 = thePoly.v[1];
	Point pt3 = thePoly.v[2];


	a = pt1.Length(pt2); //length from poly 1st point (triangle start point) to poly 2nd point (triangle 2nd point)
	b = pt2.Length(pt3); //length from poly 2nd point to poly 3rd point (triangle end point)
	c = pt3.Length(pt1); //length from poly 3rd point to poly 1st point
	s = (a + b + c) / 2;
	...
	if (numTriangle > 1) {
		for (i = 2; i < thePoly.numSides - 1; i++) {
			Point pt2 = thePoly.v[i];
			Point pt3 = thePoly.v[i+1];

			a = pt1.Length(pt2); //length from poly 1st point (triangle start point) to poly point i+1 (triangle 2nd point)
			b = pt2.Length(pt3); //length from poly point i+1 to poly point i+2 (triangle end point)
			c = pt3.Length(pt1); //length from poly point i+2 to poly 1st point
			s = (a + b + c) / 2;
