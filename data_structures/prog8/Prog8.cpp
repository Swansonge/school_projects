...
CHANGES
09-29-2108 gpc - Add ClrScr() as needed to fix show tree display.
09-29-2018 gpc - Created for spring 2018
04-06-2018 ers - I needed to comment out clrscr() so the output would display exactly like sample run
*/
#include <iostream>
#include <fstream>
#include <string>
...
PURPOSE
Convert letters to upper case and delete non-alphabetic
characters from a string.

INPUT PARAMETERS
word  -- the word to capitialize
*/
string Capitalize(string word)
{
  string result = "";
  ...
  for(unsigned i=0; i<word.length(); ++i)
    if (isalpha(word[i]))
      result += (char) toupper(word[i]); // gpc - 11-23-2017
...
int main()
{
	...
	char cmdLetter; // The current command letter
		...
		if (cmdLetter != ' ')
		{
           ...
           // clrscr();
            ...
            //clrscr();
				...
				break;
#if !defined(NoGraphics) // Enable Show Tree for Windows
         case ShowTreeCmd:
		      wordTree.ShowTree();
            break;
#endif
...
char GetCmd()
{
  ...
  if (cin.peek() == '\n')
	...
	cmdLetter = (char) toupper(cin.get()); // gpc - 11-23-2017
