...
#Network Design
#Phase 4 - UDPReceiver.py: Will receive the data stream from the UDP sender (the bytes of a BMP image), convert from from bytes back to BMP format, and display the image. Packets
#                          are sent over RDT3.0. The receiver will check the packets with a checksum to see if it is corrupt. It will also compare the sequence of the current
#                          packet received with the sequence # of the previous packet to check for duplicates. The previous ACK will be resent when the checksum or sequence number
...
def corrupt_packet(packet):
    ...
    for i in range(0, len(corruptPackets)):
        if numReceived == corruptPackets[i]:
